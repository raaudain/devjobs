...
def get_jobs(url: str, date:str, company: str, position: str, location: str, logo: str, source: str, source_url: str):
    ...
    post_date = datetime.timestamp(datetime.strptime(str(d), "%Y-%m-%d %H:%M:%S"))
...
def get_results(item: str):
    ...
    if jobs:
        for data in jobs:
            ...
            logo = data["companyLogo"].strip() if len(data["companyLogo"]) > 0 else "https://fullstackjob.com/img/icons/favicon-32x32.png"
            position = data["position"].strip()
            location = f"{data['location'].strip()}, " if len(data["location"]) > 0 else ""
            ...
            get_jobs(apply_url, date, company_name, position, locations_string, logo, source, source_url)
