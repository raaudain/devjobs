...
def getResults(item, name):
    ...
    for d in data:
        if "Engineer" in d["name"] or "Tech" in d["name"] or "Data" in d["name"] or "Software" in d["name"] or "IT" in d["name"] or "Information" in d["name"] or "Game Development" in d["name"] or "Programming" in d["name"] or "Quality Assurance" in d["name"] or "QA" in d["name"]:
            if d["jobs"]:
                jobs.extend(d["jobs"])
