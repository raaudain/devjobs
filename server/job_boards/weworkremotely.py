...
def get_results(item: str):
    ...
    try:
        ...
        for job in results:
            date = job.find("time")["datetime"].replace("T", " ").replace("Z", "") if job.find(
                "time") else datetime.strftime(datetime.now(), "%Y-%m-%d %H:%M:%S")
            post_date = datetime.timestamp(
                datetime.strptime(date, "%Y-%m-%d %H:%M:%S"))
            position = job.find("span", class_="title").text.strip()
            company_name = job.find("span", class_="company").text.strip()
            apply_url = "https://www.weworkremotely.com" + \
                job.find_all("a", href=True)[1]["href"]
