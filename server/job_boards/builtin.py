...
def get_results(item: str):
    ...
    for d in list(data.values()):
        if d["company"] not in scraped:
            ...
            logo = f"https://cdn.{base_url.replace('https://www.','')}/cdn-cgi/image/fit=scale-down,sharpen=0.3,f=auto,q=100,w=100,h=100/sites/{base_url.replace('https://','')}/files/{d['logo']}" if d["logo"] else None
